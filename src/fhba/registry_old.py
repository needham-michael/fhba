...
def _load_hls_bands(granule_files, band_list, area_def, resampler='nearest'):
    ...
    for band in band_list:
        ...
        # Remove CF encoding attributes so xarray does NOT re-apply the
        # scale_factor when the NetCDF is read back (the scaling has already
        # been applied above; leaving scale_factor in attrs causes xarray's
        # CF decoder to silently divide all values by 10 000 on reload).
...
class GranuleManager:
    ...
    def download_cloud_mask_granules(self, cloud_mask_granule_search_results=None, date=None,
        ...
        if len(cloud_mask_granule_search_results) == 0:
            logger.info("No cloud mask granules found for %s %s on %s. Skipping download.",
                        self.satellite_name, self.instrument.upper(), date)
            if date is not None:
    ...
    def retrieve_worldview_image(self, date, out_path, overwrite=False, truecolor=True):
        """Download a true-color preview image for *date*.

        * VIIRS (Suomi-NPP, NOAA-20, NOAA-21): fetched via the NASA Worldview
          Snapshot API (wvs.earthdata.nasa.gov).
        * Landsat-8 / Landsat-9: the Worldview Snapshot API does not carry
          daily Landsat imagery, so browse thumbnails are retrieved directly
          from the publicly-accessible LPDAAC S3 bucket via a CMR granule
          search.
        * MODIS and other satellites: not supported; a warning is logged and
          the method returns without downloading.
        """
        if os.path.exists(out_path) and not overwrite:
        ...
        if self.satellite_name in _viirs_layer:
            ...
            url = (
                ...
                f"&BBOX={bbox}"
                f"&CRS=EPSG:4326"
                f"&LAYERS={layer},Coastlines_15m"
                f"&WRAP=day,x"
                f"&FORMAT=image/jpeg"
                f"&WIDTH=1138&HEIGHT=1820"
                f"&colormaps=,"
            )
...
class GranuleRegistry:
    ...
    def from_dict(self, data):
        ...
        # Guard: if a GranuleManager was serialised before its band list was
        # populated (e.g. Landsat support added after initial registry creation),
        # full_band_list will be [] in the JSON.  Recover it from the
        # registry-level band-list that is always populated from config.yaml.
...
class Registry:
    ...
    def from_dict(self, data):
        ...
        # This ensures that fields added in newer versions of the app (e.g. Landsat
        # support, NBR/NDVI bands, MODIS cloud mask names) are always up to date,
        # even when the saved registry.json predates those additions.
        self.read_config()
        _config_fields = {
